...
interface TimerState {
  ...
  addManualSession: (duration: number, subjectId?: string, notes?: string, date?: string) => Promise<boolean>
}
...
export const useTimerStore = create<TimerState>((set, get) => ({
  ...
  addManualSession: async (duration: number, subjectId?: string, notes?: string, date?: string) => {
    try {
      const result = await (window as any).electron.ipcRenderer.invoke('sessions:createManual', {
        userId: TEMP_USER_ID,
      ...
      if (result.success) {
        // Refresh today's sessions to update stats immediately
