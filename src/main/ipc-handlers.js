...
ipcMain.handle('subjects:getAll', async (_, userId) => {
    try {
        const subjects = findMany(`
      SELECT s.*, 
...
ipcMain.handle('subjects:create', async (_, data) => {
    try {
        ...
        execute(`INSERT INTO Subject (id, userId, name, description, color, icon, targetDate, archived, createdAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)`, params);
...
// Update a subject
ipcMain.handle('subjects:update', async (_, id, data) => {
    try {
        const updates = [];
        const values = [];
...
// Delete a subject
ipcMain.handle('subjects:delete', async (_, id) => {
    try {
...
ipcMain.handle('subjects:getById', async (_, id) => {
    try {
        const subject = findOne('SELECT * FROM Subject WHERE id = ?', [id]);
        if (!subject) {
    ...
    `, [id]);
      ...
      `, [module.id]);
            ...
            const topicsWithResources = topics.map((topic) => ({
                ...topic,
                resources: findMany('SELECT * FROM Resource WHERE topicId = ?', [topic.id])
            }));
...
// ========== MODULE OPERATIONS ==========
// Create a module
ipcMain.handle('modules:create', async (_, data) => {
    try {
        const id = uuidv4();
        execute(`INSERT INTO Module (id, subjectId, name, description, \`order\`, createdAt)
         VALUES (?, ?, ?, ?, ?, ?)`, [
            id,
            data.subjectId,
            data.name,
        ...
        const module = findOne('SELECT * FROM Module WHERE id = ?', [id]);
...
// Update a module
ipcMain.handle('modules:update', async (_, id, data) => {
    try {
        const updates = [];
        const values = [];
        if (data.name !== undefined) {
...
ipcMain.handle('modules:bulkImport', async (_, data) => {
    try {
        const results = [];
        ...
        for (const moduleData of data.modules) {
           ...
           VALUES (?, ?, ?, ?, ?, ?)`, [
            ...
            const module = findOne('SELECT * FROM Module WHERE id = ?', [moduleId]);
            ...
            if (moduleData.topics && moduleData.topics.length > 0) {
                ...
                for (const topicData of moduleData.topics) {
                    ...
                    execute(`INSERT INTO Topic (id, moduleId, name, description, "order", completed, isImportant, totalDuration, watchedDuration, createdAt)
               VALUES (?, ?, ?, ?, ?, 0, 0, null, 0, ?)`, [
                        topicId,
                        moduleId,
                        topicData.name,
                    ...
                    const topic = findOne('SELECT * FROM Topic WHERE id = ?', [topicId]);
                    if (topic)
                        topics.push(topic);
                }
            }
...
// ========== TOPIC OPERATIONS ==========
// Create a topic
ipcMain.handle('topics:create', async (_, data) => {
    try {
        const id = uuidv4();
        execute(`INSERT INTO Topic (id, moduleId, name, description, "order", completed, isImportant, totalDuration, watchedDuration, createdAt)
         VALUES (?, ?, ?, ?, ?, 0, ?, ?, 0, ?)`, [
        ...
        const topic = findOne('SELECT * FROM Topic WHERE id = ?', [id]);
...
// Update a topic
ipcMain.handle('topics:update', async (_, id, data) => {
    try {
        const updates = [];
        const values = [];
        if (data.name !== undefined) {
...
// Delete a topic
ipcMain.handle('topics:delete', async (_, id) => {
    try {
...
ipcMain.handle('courses:getAll', async (_, userId) => {
    try {
        const courses = findMany('SELECT * FROM Course WHERE userId = ? ORDER BY createdAt DESC', [userId]);
        // Enrich courses with classes and batches
        const enrichedCourses = courses.map((course) => {
            const classes = findMany('SELECT * FROM Class WHERE courseId = ? ORDER BY `order` ASC', [course.id]);
            const batches = findMany('SELECT * FROM Batch WHERE courseId = ?', [course.id]);
...
ipcMain.handle('courses:create', async (_, data) => {
    try {
        const id = uuidv4();
        execute(`INSERT INTO Course (id, userId, name, platform, instructor, description, color, totalClasses, attendedClasses, createdAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?)`, [
            id,
            data.userId,
            data.name,
...
// Update a course
ipcMain.handle('courses:update', async (_, id, data) => {
    try {
        const updates = [];
        const values = [];
        if (data.name !== undefined) {
...
// Delete a course
ipcMain.handle('courses:delete', async (_, id) => {
    try {
...
ipcMain.handle('courses:getById', async (_, id) => {
    try {
        const course = findOne('SELECT * FROM Course WHERE id = ?', [id]);
        if (!course) {
...
ipcMain.handle('classes:create', async (_, data) => {
    try {
        const id = uuidv4();
        execute(`INSERT INTO Class (id, courseId, batchId, title, description, type, scheduledAt, videoUrl, duration, "order", status, watchedDuration, completionPercentage, isImportant, createdAt, updatedAt)
...
// Update a class
ipcMain.handle('classes:update', async (_, id, data) => {
    try {
        const updates = [];
        const values = [];
        ...
        const previousClassItem = findOne('SELECT * FROM Class WHERE id = ?', [id]);
...
// Delete a class
ipcMain.handle('classes:delete', async (_, id) => {
    try {
        const classItem = findOne('SELECT * FROM Class WHERE id = ?', [id]);
        if (classItem) {
...
ipcMain.handle('app:restoreBackup', async (_, backupData) => {
    try {
        ...
        transaction(() => {
            ...
            if (Array.isArray(backupData.subjects)) {
                const stmt = 'INSERT INTO Subject (id, userId, name, description, color, icon, targetDate, archived, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
            ...
            if (Array.isArray(backupData.courses)) {
                const stmt = 'INSERT INTO Course (id, userId, name, platform, instructor, description, color, totalClasses, attendedClasses, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
            ...
            if (Array.isArray(backupData.modules)) {
                const stmt = 'INSERT INTO Module (id, subjectId, name, description, "order", createdAt) VALUES (?, ?, ?, ?, ?, ?)';
            ...
            if (Array.isArray(backupData.batches)) {
                const stmt = 'INSERT INTO Batch (id, courseId, name, startDate, endDate) VALUES (?, ?, ?, ?, ?)';
            ...
            if (Array.isArray(backupData.topics)) {
                const stmt = 'INSERT INTO Topic (id, moduleId, name, description, "order", completed, isImportant, totalDuration, watchedDuration, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
            ...
            if (Array.isArray(backupData.classes)) {
                const stmt = 'INSERT INTO Class (id, courseId, batchId, title, description, type, scheduledAt, videoUrl, duration, "order", status, watchedDuration, completionPercentage, isImportant, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
            ...
            if (Array.isArray(backupData.resources)) {
                const stmt = 'INSERT INTO Resource (id, topicId, title, type, url, thumbnail, duration, metadata, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
            ...
            if (Array.isArray(backupData.studySessions)) {
                const stmt = 'INSERT INTO StudySession (id, userId, subjectId, startTime, sessionType, duration, notes, focusScore, endTime, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
ipcMain.handle('sessions:create', async (_event, data) => {
    try {
        const id = uuidv4();
        execute(`INSERT INTO StudySession (id, userId, subjectId, startTime, sessionType, duration, createdAt)
       VALUES (?, ?, ?, ?, ?, 0, ?)`, [
            id,
            data.userId,
            data.subjectId || null,
...
ipcMain.handle('sessions:getToday', async (_event, userId) => {
    try {
        ...
        const sessions = findMany('SELECT * FROM StudySession WHERE userId = ? AND datetime(startTime) >= datetime(?) ORDER BY startTime DESC', [userId, today.toISOString()]);
...
ipcMain.handle('sessions:getWeek', async (_event, userId) => {
    try {
        ...
        const sessions = findMany('SELECT * FROM StudySession WHERE userId = ? AND datetime(startTime) >= datetime(?) ORDER BY startTime DESC', [userId, weekAgo.toISOString()]);
        return { success: true, data: sessions };
    }
    catch (error) {
        return { success: false, error: String(error) };
...
// Create a manual focus session (user-entered duration)
ipcMain.handle('sessions:createManual', async (_event, data) => {
    try {
        const id = uuidv4();
        let endTime;
        if (data.date) {
            // Use end of the specified date
            const d = new Date(data.date + 'T23:59:59');
            endTime = d;
        } else {
            endTime = new Date();
        }
        const startTime = new Date(endTime.getTime() - data.duration * 1000);
        execute(`INSERT INTO StudySession (id, userId, subjectId, startTime, endTime, sessionType, duration, notes, createdAt)
         VALUES (?, ?, ?, ?, ?, 'focus', ?, ?, ?)`, [
            id,
            data.userId,
            data.subjectId || null,
...
ipcMain.handle('timer:saveSession', async (_, data) => {
    try {
        const id = uuidv4();
        const now = new Date().toISOString();
        execute(`INSERT INTO StudySession (id, userId, subjectId, duration, sessionType, endTime, createdAt)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [id, data.userId, data.subjectId || null, data.duration, data.type, now, now]);
        const session = findOne('SELECT * FROM StudySession WHERE id = ?', [id]);
        return { success: true, data: session };
    }
    catch (error) {
...
ipcMain.handle('timer:getSessions', async (_, filters) => {
    try {
        let query = 'SELECT s.*, subj.name as subjectName, subj.color as subjectColor FROM StudySession s LEFT JOIN Subject subj ON s.subjectId = subj.id WHERE s.userId = ?';
...
ipcMain.handle('timer:getSessionStats', async (_, userId) => {
    try {
        // Get total focus time - use sessionType instead of type
        const focusSessions = findMany(`SELECT duration FROM StudySession WHERE userId = ? AND sessionType IN ('focus', 'pomodoro')`, [userId]);
        ...
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const sessionsToday = findOne(`SELECT COUNT(*) as count FROM StudySession WHERE userId = ? AND sessionType IN ('focus', 'pomodoro') AND endTime IS NOT NULL AND datetime(endTime) >= datetime(?)`, [userId, today.toISOString()])?.count || 0;
        // Get sessions this week
        const weekAgo = new Date();
        weekAgo.setDate(weekAgo.getDate() - 7);
        weekAgo.setHours(0, 0, 0, 0);
        const sessionsThisWeek = findOne(`SELECT COUNT(*) as count FROM StudySession WHERE userId = ? AND sessionType IN ('focus', 'pomodoro') AND endTime IS NOT NULL AND datetime(endTime) >= datetime(?)`, [userId, weekAgo.toISOString()])?.count || 0;
        return {
            success: true,
            data: {
                totalFocusMinutes,
...
ipcMain.handle('timer:getWeeklyHistory', async (_, userId) => {
    try {
        const weekAgo = new Date();
        ...
        const sessions = findMany(`SELECT duration, endTime FROM StudySession WHERE userId = ? AND sessionType IN ('focus', 'pomodoro') AND endTime IS NOT NULL AND datetime(endTime) >= datetime(?) ORDER BY endTime ASC`, [userId, weekAgo.toISOString()]);
...
ipcMain.handle('ai:send-message', async (_, userId, message, userName, examName, examDate, responseLength) => {
    try {
        const response = await aiService.sendMessage(userId, message, userName, examName, examDate, responseLength);
        return response;
    }
    catch (error) {
...
ipcMain.handle('ai:get-insights', async (_, userId) => {
    try {
...
// Create a resource
// Create a resource
ipcMain.handle('resources:create', async (_, data) => {
    try {
        const id = uuidv4();
        const now = new Date().toISOString();
        execute('INSERT INTO Resource (id, topicId, title, type, url, thumbnail, duration, metadata, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [
            id,
            data.topicId,
            data.title,
            data.type,
...
ipcMain.handle('sessions:delete', async (_, id) => {
    try {
        execute('DELETE FROM StudySession WHERE id = ?', [id]);
        return { success: true };
    }
    catch (error) {
