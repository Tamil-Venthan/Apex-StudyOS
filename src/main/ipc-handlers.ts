...
ipcMain.handle(
  ...
  async (
    ...
    try {
      const id = uuidv4()
      const now = new Date()
      const startTime = new Date(now.getTime() - data.duration * 1000)

      execute(
        `INSERT INTO StudySession (id, userId, subjectId, startTime, endTime, sessionType, duration, notes, createdAt)
         VALUES (?, ?, ?, ?, ?, 'focus', ?, ?, ?)`,
        [
          id,
          data.userId,
