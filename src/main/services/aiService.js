...
export class AIService {
    ...
    `)
      ...
      `)
        ...
        `)
                ...
                importantTopics += topics.filter((t) => t.isImportant).length;
            });
    ...
    `)
        ...
        const processedCourses = courses.map((course) => {
            return {
                ...
                attendanceRate: course.totalClasses > 0
     ...
     */
    async sendMessage(userId, message, userName, examName, examDate, responseLength) {
        if (!this.isReady()) {
            return {
                success: false,
        ...
        try {
            ...
            return await this.currentProvider.sendMessage(message, context, userName, examName, examDate, responseLength);
        }
        catch (error) {
