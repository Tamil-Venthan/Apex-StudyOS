...
// Quick-add session helper
function quickAddSession(durationMinutes) {
    try {
        const id = uuidv4();
        ...
        execute(
            `INSERT INTO StudySession (id, userId, subjectId, duration, sessionType, endTime, createdAt)
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [id, 'user-1', null, durationSeconds, 'focus', now, now]
        );
        return true;
    } catch (error) {
...
function createTray() {
    ...
    tray.setToolTip('Apex StudyOS');

    const contextMenu = Menu.buildFromTemplate([
        {
            label: '📖 Open Apex StudyOS',
            click: () => {
                if (mainWindow) {
                    mainWindow.show();
                    mainWindow.focus();
        ...
        { type: 'separator' },
        {
            label: '⚡ Quick Add 15 min',
            click: () => {
                quickAddSession(15);
                if (mainWindow) mainWindow.webContents.send('session-added');
            }
        },
        {
            label: '⚡ Quick Add 30 min',
            click: () => {
                quickAddSession(30);
                if (mainWindow) mainWindow.webContents.send('session-added');
            }
